...
function numberToArray(num: number): number[] {
  ...
  function allMatch<T>(collection: T[], predicate: (item: T) => boolean): boolean {
  ...
  function transformObjectKeys<T>(obj: { [key: string]: T }, keyFunc: (key: string) => string): { [key: string]: T } {
  ...
  function invokeAll<T>(...funcs: Array<(...args: any[]) => T>): (...args: any[]) => T[] {
    return (...args: any[]) => funcs.map(fn => fn(...args));
  }
  ...
  function average<T>(arr: T[], mapFunc: (item: T) => number): number {
